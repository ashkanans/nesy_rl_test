...
An alternative line of work enforces safety through shielding \cite{alshiekh2018safe,jansen2018safe,yang2023safe,belardinelli2025probabilistic}, where a runtime monitor prevents the agent from executing actions that may lead to unsafe states. 
...
%
In contrast, we enforce constraints at \emph{training time} through an auxiliary logical loss. Only few approaches follow this direction~\cite{STLnet,UmiliPMAI24,pseudosemantic_loss,innes2020elaborating}. Among them,~\cite{UmiliPMAI24,pseudosemantic_loss} estimate the probability of satisfying a formula via pseudoprobabilities or Monte Carlo; STLnet~\cite{STLnet} relies on a student–teacher scheme that is difficult to apply in discrete domains~\cite{UmiliPMAI24}; finally, \cite{innes2020elaborating} impose a differentiable loss computed from \ltl specifications.
